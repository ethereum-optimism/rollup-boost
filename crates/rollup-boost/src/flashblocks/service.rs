...
impl FlashblockBuilder {
    ...
    pub fn build_envelope(
        ...
        let (mut transactions, withdrawals, latest_post_exec_tx) = self.flashblocks.iter().fold(
            (Vec::new(), Vec::new(), None),
            |(mut transactions, mut withdrawals, latest_post_exec_tx), delta| {
                transactions.extend(delta.transactions.clone());
                withdrawals.extend(delta.withdrawals.clone());
                let latest_post_exec_tx = delta.post_exec_tx.clone().or(latest_post_exec_tx);
                (transactions, withdrawals, latest_post_exec_tx)
            },
        );
        if let Some(post_exec_tx) = latest_post_exec_tx {
